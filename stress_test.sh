...
ORANGE='\033[0;33m'     # Orange for sequential comparisons
GREEN='\033[0;34m'      # Green for parallel comparisons
NC='\033[0m'            # No color

# Original Sequential File Comparison
# -----------------------------------
echo -e "${ORANGE}Starting sequential comparison of individual files...${NC}" >> $LOGFILE
...
find "$TEMP_DIR_1" -type f \( -name "*.cpp" -o -name "*.h" -o -name "*.c" \) | while read -r FILE_1; do
    ...
    if [ -f "$FILE_2" ]; then
        ...
        echo -e "${ORANGE}Comparing $(basename "$FILE_1") from $BRANCH_1 against $(basename "$FILE_2") from $BRANCH_2${NC}"
        echo -e "${ORANGE}Comparing $(basename "$FILE_1") from $BRANCH_1 against $(basename "$FILE_2") from $BRANCH_2${NC}" >> $LOGFILE
        ...
        echo -e "${RED}File $FILE_2 does not exist in $TEMP_DIR_2${NC}"
...
find "$TEMP_DIR_2" -type f \( -name "*.cpp" -o -name "*.h" -o -name "*.c" \) | while read -r FILE_2; do
    ...
    if [ ! -f "$FILE_1" ]; then
        ...
        echo -e "${RED}File $FILE_1 does not exist in $TEMP_DIR_1${NC}"
...
# --------------------------------------
echo -e "${GREEN}NEW TEST - RUNNING SRCDIFF IN PARALLEL${NC}" >> $LOGFILE
...
run_srcdiff_on_pair() {
    ...
    echo -e "${GREEN}Comparing $(basename "$FILE_1") from $BRANCH_1 with $(basename "$FILE_2") from $BRANCH_2${NC}"
    echo -e "${GREEN}Comparing $(basename "$FILE_1") from $BRANCH_1 with $(basename "$FILE_2") from $BRANCH_2${NC}" >> $LOGFILE
    track_srcdiff_performance "srcdiff $FILE_1 $FILE_2 -o /dev/null"
